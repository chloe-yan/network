<script>
    ...
    function generateNetwork(nodeCount, edgeCount) {
        ...
        for (let i = 0; i < edgeCount; i++) {
            ...
            // Ensure no self-loops and no multiedges
        ...
        return {
            nodes: graph.nodes().map((node) => ({ index: parseInt(node), name: `Node ${node}`, group: Math.floor(Math.random() * 10) })),
            links: graph.edges().map((edge) => ({
                source: parseInt(graph.source(edge)),
                target: parseInt(graph.target(edge)),
            }))
        };
    }
    ...
    onMount(async function () {
        // Adjust these values to test different sizes
        const nodeCount = 1000;
        const edgeCount = 10000;

        const data = await generateNetwork(nodeCount, edgeCount);
        nodes = data.nodes;
